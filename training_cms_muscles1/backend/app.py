...
def create_app():
    ...
    # Импортируем ВСЕ нужные функции из api.py
    from backend.auth import register, login
    from backend.api import (
        get_muscle_groups,
        get_exercises,
        get_exercise,
        create_exercise,         # <-- была пропущена
        update_exercise,         # <-- была пропущена
        delete_exercise,         # <-- была пропущена
        get_exercise_versions,   # <-- была пропущена
        get_version_by_id,       # <-- была пропущена
        create_workout,
        get_my_workouts,
        get_workout,
        log_set,
        delete_workout,
        add_favorite,            # <-- была пропущена
        remove_favorite,         # <-- была пропущена
        get_favorites            # <-- была пропущена
    )
    ...
    app.add_url_rule('/api/exercises/<int:exercise_id>', view_func=update_exercise, methods=['PUT'])
    app.add_url_rule('/api/exercises/<int:exercise_id>', view_func=delete_exercise, methods=['DELETE'])
