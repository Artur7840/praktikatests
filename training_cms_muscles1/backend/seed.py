...
# Добавляем путь к корню проекта (чтобы импортировать config)
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# Пытаемся получить DATABASE_URL из конфига или окружения
try:
...
def init_db():
    """Создаёт таблицы, используя schema.sql (исправленный для PostgreSQL)."""
    conn = get_connection()
...
def insert_muscle_group(cursor, name, description, photo_url):
    """Вставляет группу мышц, если её ещё нет (по уникальному name)."""
    cursor.execute("""
...
def insert_exercise(cursor, name, description, technique, difficulty, photo_url, muscle_group_id):
    """Вставляет упражнение, если его ещё нет (проверка по name + muscle_group_id)."""
    cursor.execute("""
        SELECT id FROM exercise WHERE name = %s AND muscle_group_id = %s
    """, (name, muscle_group_id))
    if cursor.fetchone() is None:
        cursor.execute("""
            INSERT INTO exercise (name, description, technique, difficulty, photo_url, muscle_group_id)
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (name, description, technique, difficulty, photo_url, muscle_group_id))

def insert_user(cursor, email, password, role='student'):
    """Вставляет тестового пользователя, если его ещё нет."""
...
def seed_database():
    ...
    for name, desc, url in groups:
        insert_muscle_group(cursor, name, desc, url)

    print("Заполнение упражнений...")
    exercises = [
    ...
    for ex in exercises:
        insert_exercise(cursor, *ex)

    print("Заполнение тестовым пользователем...")
    insert_user(cursor, 'student@example.com', 'student', 'student')

    conn.commit()
    cursor.close()
    conn.close()
    print("✅ База данных PostgreSQL успешно заполнена (8 групп, 41 упражнение, тестовый пользователь).")
